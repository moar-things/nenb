...
// Parses a .nenb.md file into fragments, which will later be turned into
// blocks and directives.
// Fragments are either nenb headers/lists, fence blocks, or else markdown.
...
function getFragment (line) {
  if (isDirectiveLine(line)) return new DirectiveFragment()
  if (isFenceLine(line)) return new FenceFragment()
...
class MarkdownFragment extends Fragment {
  ...
  read (reader) {
    ...
    while (true) {
      ...
      if (line == null) break
      if (isDirectiveLine(line) || isFenceLine(line)) break
...
function cli () {
  ...
  if (fileName == null) fileName = path.join(path.dirname(__dirname), 'test', 'fixtures', 'fragments-basic.nenb.md')
