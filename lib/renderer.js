...
async function render (fileName, sections, outFileName) {
  ...
  await generateTrailer(context, sections, lines)

  const content = lines.join('\n')
  await fsWriteFile(outFileName, content)

  return context.files.output
}
...
async function generateTrailer (context, sections, lines) {
  ...
  if (context.moduleScript.trim() !== '') {
    const bLines = await browserify.render(context)
