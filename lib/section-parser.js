...
async function parse (fileName, content) {
  ...
  for (let fragment of fragments) {
    ...
    if (fragment.isFence()) {
      const { lang, langExtra, lines, attrs } = fragment
      const section = new Block(fileName, lineNumber, lang, langExtra, lines, attrs)
      sections.push(section)
      continue
    }
...
class Block extends Section {
  constructor (fileName, lineNumber, lang, langExtra, lines, attrs) {
    super(fileName, lineNumber)
    this.lang = lang
    this.langExtra = langExtra
    this.lines = lines
    this.attrs = attrs || {}
    this.attrs.lang = lang
    this.attrs.langExtra = langExtra
  }
